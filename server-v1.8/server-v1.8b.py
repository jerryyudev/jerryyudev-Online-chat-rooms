...
class ChatServer:
        ...
        while self.running:
            try:
                ...
                if self.running:  # Only accept new connections when the server is running
                    self.client_count += 1
                    ip, _ = client_address  # We only need the IP address
                    print(f"New user connected: {ip}")
                    threading.Thread(target=self.handle_client, args=(client_socket, ip)).start()
                else:
    ...
    def handle_client(self, client_socket, ip):
        username = ip  # Use IP address as username
    ...
    def process_message(self, client_socket, username, message):
        if message.startswith("/ip"):
            ip_port_msg = f"您的IP地址为：{client_socket.getpeername()[0]}"
